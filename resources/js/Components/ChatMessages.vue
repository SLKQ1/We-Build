<template>
    <div v-for="message in messages" :key="message.id" :id="message.id">
        <div v-if="message.user.id === $page.props.auth.user.id"
            ...
            <div>
                ...
                <span class="text-xs text-gray-300 leading-none">{{ moment(message.created_at).fromNow() }}</span>
            </div>
            <div class="flex flex-col text-center">
                <div class="flex-shrink-0 h-10 w-10 rounded-full bg-blue-300"></div>
                <p class=" text-gray-200">{{ useTruncate(message.user.name, 5, '...') }}</p>
            </div>
        </div>
